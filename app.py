...
def technical_detail(rows, title="Technical detail"):
    with st.expander(title):
        st.dataframe(
            pd.DataFrame(
                [(str(statistic), str(value)) for statistic, value in rows],
...
def show_ai_interpretation(assessment, results):
    ...
    st.write("Want a plain-English and business-oriented explanation?")
...
def risk_assessment(bundle, results):
    ...
    st.info("This assessment uses historical medical-cost patterns and is not an insurer quote, guaranteed future cost, or medical assessment.")
    ...
    with left:
        ...
        strongest_support = assessment["probabilities"].get(assessment["tier"], 0.0)
        st.metric(f"Support for {assessment['tier']}-risk classification", f"{strongest_support:.1%}")
    ...
    with right:
        ...
        typicality_text = (
            "The combined profile is reasonably similar to other profiles in the predicted group."
            if assessment["status"] == "Typical"
            else "The combined profile is relatively unusual compared with other profiles in the predicted group."
        )
